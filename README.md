...
- **效能優化**：近戰攻擊事件改讀一次 PDC id 直接查表（原本逐武器模組各複製一份 ItemMeta）；盾牌 tick 先做材質檢查，未持盾玩家不再讀 ItemMeta；屬性系統的 NamespacedKey 改為建構時快取。
- **give 指令註冊表化**：`/getego give` 與自取分支共用同一份物品註冊表，tab-complete 由註冊表自動推導（改為字母排序）。
...
Callisto 用自身骨骼鍛造的巨劍。「聽見了嗎？那由提比婭的一對肱骨與二十四根肋骨奏響的旋律！」
...
| **射手本人**聽自己的預測音 | Fabric 客戶端 mod 攔截 `SoundManager.play` | 玩家自裝 [fabric-1.0.1](https://github.com/Crossing-Dead-Development/Limbus-E.G.O-Weapons/releases/tag/fabric-1.0.1) |
...
- 來源：[Limbus-E.G.O-weapon-plugin-ResourcePack](https://github.com/Crossing-Dead-Development/Limbus-E.G.O-weapon-plugin-ResourcePack)
- 目前版本：**v.2.17**（SHA-1 `060302e85c12d23127b7c4eb3b7050c82615e20d`）
...
3. 啟動伺服器
4.（可選）玩家安裝 [fabric-1.0.1](https://github.com/Crossing-Dead-Development/Limbus-E.G.O-Weapons/releases/tag/fabric-1.0.1) 客戶端 mod 消除自身聽到的原版弓箭預測音
